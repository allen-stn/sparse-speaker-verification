...
class SpeakerBrain(sb.core.Brain):
    ...
    def on_fit_start(self):
        super().on_fit_start()
        self._global_step = 0
        self._lam_sched = optimisers.lamda_scheduler(
            opt=self.optimizer,
            idx=0,
            warmup=self.hparams.get("lam_warmup", 0),
            increment=self.hparams.get("lam_increment", 0.05),
            cooldown=self.hparams.get("lam_cooldown", 0),
            target_sparse=self.hparams.get("target_sparsity", 0.7),
            reg_param="lamda",
        )
    ...
    def on_fit_batch_end(self, batch, outputs, loss, should_step):
        ...
        update_every = self.hparams.get("lam_update_every", 200)
