...
# Modified for Adabreg Based thesis experiments and comparison studies.
...
class SpeakerBrain(sb.core.Brain):
    ...
    def on_fit_start(self):
        super().on_fit_start()
        self._global_step = 0
        self._lam_sched = optimizers.lamda_scheduler(
            opt=self.optimizer,
            idx=0,
            warmup=getattr(self.hparams, "lam_warmup", 0),
            increment=getattr(self.hparams, "lam_increment", 0.05),
            cooldown=getattr(self.hparams, "lam_cooldown", 0),
            target_sparse=getattr(self.hparams, "target_sparsity", 0.7),
        )
    ...
    def on_fit_batch_end(self, batch, outputs, loss, should_step):
        ...
        self._global_step += 1
        sparse = compute_model_sparsity(self.modules.embedding_model)

        update_every = getattr(self.hparams, "lam_update_every", 200)
        if self._global_step % update_every == 0:
            self._lam_sched(sparse, verbosity=1)
